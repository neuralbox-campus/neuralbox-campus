...
export default function DashboardPage() {
  ...
  const { data: paymentsData } = useApi('/admin/payments?limit=3');
  const { data: usersData } = useApi('/admin/users?limit=3');
  ...
  const kpis = stats ? [
    ...
    { i: '⚡', v: stats.activeSinapsis || stats.activeSubscriptions || 0, l: 'Sinapsis' },
        ...
        <div className="nx-card fi fi2">
          ...
          {(Array.isArray(payments) ? payments : []).map((p, i) => (
            <div key={i} className="nx-row-item">
              ...
              <span className="nx-row-mono">{p.amount ? `$${(p.amount / 100).toFixed(0)}` : ''}</span>
            </div>
          ))}
        </div>
        <div className="nx-card fi fi2">
          ...
          {(Array.isArray(users) ? users : []).map((u, i) => (
            <div key={i} className="nx-row-item">
              ...
              <span className="bg bg-b">{u.role === 'ADMIN' ? 'Admin' : u.subscriptions?.length > 0 ? 'Sinapsis' : 'Free'}</span>
            </div>
          ))}
        </div>
      </div>
